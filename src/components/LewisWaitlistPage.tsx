...
import imgLewisOutfitPhoto from "figma:asset/9769fa222dfe175f60b28e6da725e764d0e83fa6.png"; // Lewis's featured edit image from landing page

interface LewisWaitlistPageProps {
  uploadedImageUrl?: string;
}

export function LewisWaitlistPage({ uploadedImageUrl }: LewisWaitlistPageProps) {
  ...
  useEffect(() => {
    ...
    if (hasProcessedState.current) {
      console.log('🔍 LewisWaitlistPage - Already processed state, skipping');
      return;
    }
    ...
    const loadIntakeData = async () => {
        ...
        if (accessToken) {
          try {
            ...
            if (response.ok) {
              ...
              // Find the most recent Lewis order
              const lewisOrders = orders.filter((o: any) => 
                o.stylist_id === 'lewis_bloyce' && 
                (o.status === 'intake_submitted' || o.status === 'waitlist')
              );
              
              console.log('📦 Lewis orders found:', lewisOrders.length);
              if (lewisOrders.length > 0) {
                console.log('📦 Most recent Lewis order:', lewisOrders[0]);
              }
              
              if (lewisOrders.length > 0) {
                const mostRecentOrder = lewisOrders[0];
                intakeData = {
      ...
      console.log('🔍 LewisWaitlistPage - Final intake data:', intakeData);
      
      // Now we expect uploadedImageUrl (string) and orderId from the saved order
      if (!intakeData || !intakeData.uploadedImageUrl || !intakeData.orderId) {
        console.warn('⚠️ No intake data found - redirecting to Lewis landing page');
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center px-4 pb-8">
        {/* Lewis's Featured Edit Image from Landing Page - Stacked Card Effect */}
        ...
        <div className="w-full max-w-[361px] mb-16">
          <p className="font-['Helvetica_Neue:Regular',sans-serif] text-[12px] leading-[24px] tracking-[1px] text-[#1e1709]">
            7 ITEM CAPSULE EDIT + STYLING NOTES CURATED BY LEWIS BLOYCE.
          </p>
        </div>
          ...
          <div className="flex-1 flex flex-col items-center px-4 pb-8">
            ...
            <div className="w-full max-w-[361px] mb-16 text-center">
              ...
              <p className="font-['Helvetica_Neue:Light',sans-serif] text-[12px] leading-[20px] tracking-[0.5px] text-[#1e1709]">
                When Lewis's waitlist opens you will get an invite.
              </p>
            </div>
